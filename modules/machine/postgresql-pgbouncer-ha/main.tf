resource "juju_application" "machine_postgresql" {
  ...
  units  = var.total_units
  constraints = var.constraints_postgresql
...
resource "juju_application" "data_integrator" {
  ...
  units  = var.total_units
  constraints = var.constraints_pgbouncer
...
resource "juju_application" "grafana_agent" {
  ...
  endpoint_bindings = var.grafana_agent_endpoints
}
...
resource "juju_application" "pgbouncer" {
  ...
  config = var.pgbouncer_config
  expose {}
  endpoint_bindings = var.pgbouncer_endpoints
}
...
resource "juju_application" "hacluster" {
  ...
  charm {
    ...
    base     = var.hacluster_charm_base
    revision = var.hacluster_charm_revision
  }
  endpoint_bindings = var.hacluster_endpoints
}
